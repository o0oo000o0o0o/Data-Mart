...
last_value = pd.read_sql('SELECT max(distribution_orderinfo_id) FROM stg_distribution_orderinfo', con=postgres_engine).iloc[0,0]
last_value
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BLC_DISTRIBUTION_ORDERINFO','stg_DISTRIBUTION_ORDERINFO'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + ","+ str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
