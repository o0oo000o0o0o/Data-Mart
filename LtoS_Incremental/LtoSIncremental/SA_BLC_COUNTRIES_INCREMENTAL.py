...
insrt_cmd = "INSERT INTO stg_COUNTRIES("+col_names+") "                 "VALUES ( %s )" % (placeholders)
...
#Log_Audit
mycursor.execute('SELECT count(country_id) FROM eBooks2go.BLC_COUNTRIES')
Total_Records_from_source = ','.join(map(str,[str(x[0]) for x in mycursor.fetchall()]))

pgcursor.execute('SELECT count(country_id) FROM stg_COUNTRIES')
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BLC_COUNTRIES','stg_COUNTRIES'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + "," + str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
