...
last_value = pd.read_sql('SELECT max(order_id) FROM stg_order', con=postgres_engine).iloc[0,0]
last_value
...
slt_cmd ="Select * from eBooks2go.BLC_ORDER where ORDER_ID > " + str(last_value)
...
insrt_cmd = "INSERT INTO stg_ORDER("+col_names+") " \
...
#Log_Audit
mycursor.execute('SELECT count(order_id) FROM eBooks2go.BLC_ORDER')
Total_Records_from_source = ','.join(map(str,[str(x[0]) for x in mycursor.fetchall()]))

pgcursor.execute('SELECT count(order_id) FROM stg_ORDER')
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BLC_ORDER','stg_ORDER'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + ","+ str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
