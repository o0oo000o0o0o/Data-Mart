...
last_value = pd.read_sql('SELECT max(customer_id) FROM stg_crm_customer', con=postgres_engine).iloc[0,0]
last_value
...
slt_cmd ="Select * from eBooks2go.CRM_CUSTOMER where CUSTOMER_ID > " + str(last_value)
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.CRM_CUSTOMER','stg_CRM_CUSTOMER'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + ","+ str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
