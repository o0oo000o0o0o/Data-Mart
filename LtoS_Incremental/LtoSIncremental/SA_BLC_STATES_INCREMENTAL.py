...
last_value = pd.read_sql('SELECT max(state_id) FROM stg_states', con=postgres_engine).iloc[0,0]
last_value
...
slt_cmd ="Select * from eBooks2go.BLC_STATES where STATE_ID > " + str(last_value)
...
insrt_cmd = "INSERT INTO stg_STATES("+col_names+") " \
...
#Log_Audit
mycursor.execute('SELECT count(state_id) FROM eBooks2go.BLC_STATES')
Total_Records_from_source = ','.join(map(str,[str(x[0]) for x in mycursor.fetchall()]))

pgcursor.execute('SELECT count(state_id) FROM stg_STATES')
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BLC_STATES','stg_STATES'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + "," +str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
