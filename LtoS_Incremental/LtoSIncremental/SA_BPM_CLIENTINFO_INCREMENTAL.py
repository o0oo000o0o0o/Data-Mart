...
last_value = pd.read_sql('SELECT max(clientid) FROM stg_bpm_client_info', con=postgres_engine).iloc[0,0]
last_value
...
slt_cmd ="Select * from eBooks2go.BPM_client_info where clientid > " + str(last_value)
...
insrt_cmd = "INSERT INTO stg_BPM_CLIENT_INFO("+col_names+") " \
...
#Log_Audit
mycursor.execute('SELECT count(clientid) FROM eBooks2go.BPM_client_info')
Total_Records_from_source = ','.join(map(str,[str(x[0]) for x in mycursor.fetchall()]))

pgcursor.execute('SELECT count(clientid) FROM stg_BPM_client_info')
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BPM_client_info','stg_BPM_client_info'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) +"," + str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
