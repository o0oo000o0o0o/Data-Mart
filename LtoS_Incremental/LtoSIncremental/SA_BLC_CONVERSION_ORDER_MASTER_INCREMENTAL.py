...
last_value = pd.read_sql('select max(conversion_order_id) from stg_CONVERSION_ORDER_MASTER', con=postgres_engine).iloc[0,0]
last_value
...
slt_cmd ="Select * from eBooks2go.BLC_CONVERSION_ORDER_MASTER where conversion_order_id > " + str(last_value)
...
New_Records_Target,Status, Remarks, Execution_time) Values ('LtoS','eBooks2go.BLC_CONVERSION_ORDER_MASTER','stg_CONVERSION_ORDER_MASTER'," + str(Total_Records_from_source) + "," + str(Total_Records_from_target) + ","+ str(new_records_source) + "," + str(new_records_target) + ",'Completed','" + str(count_failed) + " Records Failed.Incremental Data Update','" + str(round(time.time() - start_time,2))+" seconds')"
